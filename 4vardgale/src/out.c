...
void ausmflux(REAL *ql, REAL *qr, REAL *fl) {
  ...
  UINT i;
  REAL rho_left, rho_right, u_left, u_right, p_left, p_right, c_left, c_right,
    c_mean, h_left, h_right, amach_l, amach_r, alpha_l, alpha_r, temp1, temp2,
    u_lp, u_rm, p_lp, p_rm, ak_half, p_half, s_half, flux_v1, flux_d1, flux_p,
    flux_n, p_min, a_min, gam, gam1;
  ...
  if (amach_l <= 1)
    u_lp =
      alpha_l * (0.25 * temp1 * temp1 / c_mean - 0.5 * temp2) + 0.5 * temp2;
  else
    u_lp = 0.5 * temp2;
  ...
  if (amach_r <= 1)
    u_rm =
      alpha_r * (-0.25 * temp1 * temp1 / c_mean - 0.5 * temp2) + 0.5 * temp2;
  else
    u_rm = 0.5 * temp2;
  ...
  //......enthalpy flux
  fl[2] = 0.5 * (fl[0] * (h_left + h_right) - fabs(fl[0]) * (h_right - h_left));
  ...
  flux_d1 =
    0.5 * (fl[0] * (u_left + u_right) - fabs(fl[0]) * (u_right - u_left));
